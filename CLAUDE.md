...
- 기능: 하이브리드 의미검색(로컬), 정적 키워드검색(공개), 인기 Best, **분야 카드 그리드 + 목적 컬렉션**,
  카테고리 클릭 시 **"이렇게 쓰면 이런 결과" 비포/애프터 데모**(코드/텍스트 17개=실제 입출력 스니펫,
  비주얼 5개=손으로 그린 SVG 결과 목업), 카드별 **"사용법" 펼침**, 원클릭 **설치 복사**,
  안전/신선도/검증/스타 **뱃지**, 라이트/네이비-다크 + 테마 토글, 로고→홈.
...
- 카테고리 데모 다듬기: 스니펫 문구·SVG 목업 디테일(`data/category_demos.json`·`data/demos/*.svg`).
